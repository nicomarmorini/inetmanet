...
/*			if ((state->lastTsnAck == state->lastTSN || (getOutstandingBytes()==0 && transmissionQ->getQueueSize()==0 && scount==0)) && fsm->getState() == SCTP_S_SHUTDOWN_PENDING)*/
			if ((getOutstandingBytes()==0 && transmissionQ->getQueueSize()==0 && scount==0) && fsm->getState() == SCTP_S_SHUTDOWN_PENDING)
			{
		...
		case SHUTDOWN_ACK:
			...
			if (fsm->getState()!=SCTP_S_ESTABLISHED)
			{
				...
				sctpEV3<<"state="<<stateName(fsm->getState())<<"\n";
...
SCTPEventCode SCTPAssociation::processSackArrived(SCTPSackChunk* sackChunk)
{
	...
	sctpEV3 << "\n--------------------------\n"
           << simulation.getSimTime() << " processSackArrived at "
           << localAddr   << ": tsna=" << tsna
           << " arwnd="   << arwnd
           << " numGaps=" << numGaps
           << " numDups=" << numDups << endl << " -> "
           << " osb on path " << pathId << "=" << path->outstandingBytes
           << " highestTsnReceived=" << state->highestTsnReceived
           << " highestTsnAcked="    << state->highestTsnAcked
           << " lastTsnAcked="       << state->lastTsnAck << "\n";
   ...
   sctpEV3 << " tsna=" << tsna << endl;
   for (int32 j=0; j<numGaps; j++)
   {
      sctpEV3 << "  - Gap: " << sackChunk->getGapStart(j) << " - " << sackChunk->getGapStop(j) << "\n";
   }
/*
   sctpEV3 << "Retransmission Queue:" << endl;
   for(SCTPQueue::PayloadQueue::iterator pi = retransmissionQ->payloadQueue.begin(); pi != retransmissionQ->payloadQueue.end(); pi++) {
      SCTPDataVariables* datVar = pi->second;
      sctpEV3 << "   TSN " << datVar->tsn
              << "  acked=" << datVar->hasBeenAcked
              << "  path=" << datVar->lastDestination
              << "  countsAsOutstanding=" << datVar->countsAsOutstanding << endl;
   }
*/

   osbPathBefore = path->outstandingBytes;
   osbBefore = getOutstandingBytes();
   sctpEV3<<"osb="<<osbPathBefore<<", osbPathBefore="<<osbBefore<<"\n";
   ...
   if (numGaps > 0 && !retransmissionQ->payloadQueue.empty())
   {
         ...
         for (uint32 pos=lo; pos <= hi; pos++)
         {
            ...
            if (datVar)
            {
               ...
               if (datVar->hasBeenAcked == false && datVar->hasBeenAbandoned == false && datVar->hasBeenRemoved == false)
               {
                     ...
                     sctpEV3 << simulation.getSimTime() << ": " << datVar->tsn
                             << " added to newly acked bytes:  " << ackedBytes << "\n";
      ...
      for (int32 key=0; key<numGaps; key++)
      {
         ...
         for (uint32 i = lo+1; i<=hi-1; i++)
         {
            ...
            if (pq != retransmissionQ->payloadQueue.end())
            {
               sctpEV3 << "TSN " << pq->second->tsn << " (last " << pq->second->lastDestination << ")\t";
               if (pq->second->hasBeenAcked)
               {
               ...
               else
               {
                  ...
                  const uint32 chunkReportedAsMissing = (hiAcked > pq->second->tsn) ? 1 : 0;
                  if (chunkReportedAsMissing > 0)
                  {
                     ...
                     printf("GapReports for TSN %u [ret=%d,fast=%s] at t=%s:   %d --> %d by %d\n",
                            pq->second->tsn,
                            pq->second->numberOfRetransmissions,
                            (pq->second->hasBeenFastRetransmitted == true) ? "YES" : "no",
                            simTime().str().c_str(),
                            pq->second->gapReports,
                            pq->second->gapReports + chunkReportedAsMissing,
   ...
   /* update RTT measurement for newly acked data chunks */
      sctpEV3 << simulation.getSimTime() << ": SACK: rtt=" << rttEst
              << ", ackedBytes=" << ackedBytes << ", pathId=" << pathId << "\n";
   ...
   sctpEV3 << "Before ccUpdateAfterSack for path " << path->remoteAddress << " with tsna=" << tsna << ":" << endl
           << "rtxNecessary=" << ((rtxNecessary == true) ? "YES!" : "no") << " (on path " << path->remoteAddress << ")" << endl;
...
uint32 SCTPAssociation::dequeueAckedChunks(uint32 tsna, IPvXAddress pathId, simtime_t* rttEstimation)
{
   uint32 newlyAckedBytes = 0;
   SCTPDataVariables* chunk = NULL;
   simtime_t timeDiff = SIMTIME_ZERO;
      ...
      if (tsnGe(tsna,chunk->tsn))
      {
         ...
         if (chunk->hasBeenAcked == false && chunk->countsAsOutstanding)
         {
            ...
            if (chunk->numberOfTransmissions == 1 && chunk->lastDestination == pathId)
            {
               ...
               if (timeDiff < *rttEstimation || *rttEstimation == MAXTIME)
...
SCTPEventCode SCTPAssociation::processDataArrived(SCTPDataChunk* dataChunk, uint32 chunkCount)
{
	...
	if (state->newChunkReceived)
	{
		...
		int ret = iter->second->enqueueNewDataChunk(makeVarFromMsg(dataChunk));
		if (ret > 0)
		{
...
{
	simtime_t rttEstimate = SIMTIME_ZERO;
	IPvXAddress addr;
	simtime_t hbTimeField = SIMTIME_ZERO;
	...
	sctpEV3<<"Received HB ACK chunk...resetting error counters on path "<<(addr)<<", rtt estimate: "<<rttEstimate<<"\n";

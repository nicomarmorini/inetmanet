...
//
// Copyright (C) 2006 Andras Varga and Levente
//
...
void Ieee80211gMac::initialize(int stage)
{
	...
	time_start_log = par("time_start_log");
	EV <<" MAC start log at: "<< time_start_log;
	...
	//scheduleAt(0, endTimeout);
        ...
        cwStats.setName("Contention Window");
        thStats.setName("Received Throughput");
        frameLengths.setName("Frame Lengths");
	/*
	...
	if(!transmissionQueue.empty())
	{
        ...
        FSMA_State(WAITACK)
        {
            ...
            FSMA_Event_Transition(Receive-ACK,
                                  ...
                                  IDLE,
                if (retryCounter == 0 && simTime() > time_start_log) numSentWithoutRetry++;
                if (simTime() > time_start_log) {
               	 numSent++;
               	 frameLengths.collect(getCurrentTransmission()->getByteLength());
                }
                cancelTimeoutPeriod();
                if (dynamic_cast<Ieee80211DataFrame *>(transmissionQueue.front())) {
               	 Ieee80211DataFrame *temp = check_and_cast<Ieee80211DataFrame *> (transmissionQueue.front());
               	 serviceTime.collect(simTime()-temp->getTimestamp());
               	 EV << "Il frame con tempo di servizio iniziale pari a " << temp->getTimestamp() << " e' stato inviato con successo!" << endl;
               	 EV << "Il tempo di servizio totale e' stato pari a " << simTime()-temp->getTimestamp() << endl;
               	 // delete temp;
                }
	 ...
	 FSMA_Event_Transition(Transmit-Broadcast,
        ...
        FSMA_State(RECEIVE)
        {
            FSMA_No_Event_Transition(Immediate-Receive-Error,
                ...
                if (simTime() > time_start_log && msgKind == COLLISION)
			numCollision++;
				if (fixFSM)
					finishReception();
				else
...
double Ieee80211gMac::computeFrameDuration(int bits, double bitrate)
{
	...
	if(opMode=='g') {
		...
		int k = ceil((16+bits+6)/(bitrate/1e6*4));
		int n = (16+bits+6);
		int d = (bitrate/1e6*4);
		EV << "";
	}
...
void Ieee80211gMac::finish()
{
	...
	recordScalar("Mean Frame Length", frameLengths.getMean());
	recordScalar("Frame Length stddev", frameLengths.getStddev());
	recordScalar("Sent Throughput", ((double)frameLengths.getSum()*8)/simTime()/1.0e6);
	recordScalar("CW medio", cwStats.getMean());
	recordScalar("CW max", cwStats.getMax());
	recordScalar("CW dev", cwStats.getStddev());
	recordScalar("Mean service time", serviceTime.getMean());

	recordScalar("numSent", numSent);
	recordScalar("numSentWithoutRetry",numSentWithoutRetry);
	recordScalar("numReceived", numReceived);
	recordScalar("numSelfReceived", numSelfReceived);

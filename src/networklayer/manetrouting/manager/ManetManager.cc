...
bool manetPurgeRoutingTables=false;
	if (stage==4)
	{
      ...
      else if (strcmp("GEO", routingProtocol)==0)
			{
				if (!gate("to_geo")->isConnected())
				{
					dynamicLoad = true;
					cModuleType *moduleType = cModuleType::find("inet.networklayer.manetrouting.GEO");
					routingModule = moduleType->create("manetroutingprotocol", this);
					   // Connet to ip
					routingModule->gate("to_ip")->connectTo(gate("from_geo"));
				}
			}

...
class INET_API RoutingTable: public cSimpleModule, public IRoutingTable, protected INotifiable
{
  ...
  public:
    ...
    /**
     * Finds the first route with the given parameters. returns an array
     * with all possible routes. It is needed to mangle route metrics. 
     */
    virtual std::multimap<const IPRoute *, int> findRouteList(const IPAddress& target,
        const IPAddress& netmask, const IPAddress& gw, int metric = 0, const char *dev = NULL);

    /** Update route metric, returns 0 if no route was found and 1
     * otherwise
     */

    virtual int setMetric(int routeindex, int newmetric);
    ...
    /** 
     *  flush all the routing table (unicast and multicast)
     */
    ...
    virtual bool scrambleRoutes(const IPAddress&, int maxMetric = 10);
};
